...
const AllUsersPage = () => {
  ...
  const { data, refetch, isLoading, isRefetching, fetchNextPage, hasNextPage } =
    useInfiniteQuery({
      queryKey: ["users-admin"],
      queryFn: async ({ pageParam = 0 }) => {
        const res = await axiosSecure.get(
          `/users-admin?limit=9&offset=${pageParam}`
        );
        return { ...res?.data, prevOffset: pageParam };
      },
      getNextPageParam: (lastPage) => {
        const nextOffset = lastPage.prevOffset + 9;
        if (nextOffset >= lastPage.countUsers) {
          return undefined;
        }
        return nextOffset;
      },
            ...
            <div>
              <thead className="whitespace-nowrap">
                <tr>
                  ...
                  <th className="p-4 w-full pr-10 text-right text-sm font-semibold text-black">
                    Status
                    <svg
